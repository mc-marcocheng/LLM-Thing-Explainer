...
class StateMachineLogitsProcessor(LogitsProcessor):
    ...
    Args:
        no_prefix_space_word_token_lists (`list[list[int]]`):
            A list of token lists without prefix spaces, where each sublist defines a set of valid token transitions.
        prefix_space_word_token_lists (`list[list[int]]`):
            A list of token lists with prefix spaces, where each sublist defines a set of valid token transitions.
        special_token_lists (`list[list[int]]`):
            A list of token lists for special tokens, where each sublist defines a set of valid token transitions.
    ...
    Examples:
        ...
        >>> processor = StateMachineLogitsProcessor(
        ...     no_prefix_space_word_token_lists=[[0, 1]],
        ...     prefix_space_word_token_lists=[[2, 3, 4]],
        ...     special_token_lists=[[5]]
        ... )
    ...
    def __init__(
        ...
        self.token_state_machine = TokenStateMachine(
            no_prefix_space_word_token_lists,
            prefix_space_word_token_lists,
            special_token_lists,
        )
